...
export function PaintDeathBoothPage() {
    ...
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-4xl font-bold text-destructive mb-4">Booth Not Found</h1>
  ...
  const paintMenu = booth.menu.filter(item => item.description.includes('(Paint Side)'));
  const deathMenu = booth.menu.filter(item => item.description.includes('(Death Side)'));
        ...
        <div className="space-y-8">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/30 rounded-lg p-6">
              <h2 className="text-3xl font-bold text-primary mb-6">Paint Side Menu</h2>
              <div className="space-y-4">
                {paintMenu.map((item, index) => (
            ...
            <div className="bg-gradient-to-br from-destructive/10 to-destructive/5 border border-destructive/30 rounded-lg p-6">
              <h2 className="text-3xl font-bold text-destructive mb-6">Death Side Menu</h2>
              <div className="space-y-4">
                {deathMenu.map((item, index) => (
                  <div key={index} className="border-b border-destructive/20 pb-4 last:border-0">
                    <h3 className="text-xl font-semibold text-foreground mb-2">
                      {item.name}
                    </h3>
                    <p className="text-sm text-muted-foreground">
